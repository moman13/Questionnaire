@if($status == 1)
    <main class="flex-1 relative z-0 overflow-y-auto pt-2 pb-6 focus:outline-none md:py-6" tabindex="0" x-data x-init="$el.focus()">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8  ">
            <h1 class="text-2xl font-semibold text-gray-900">Offer</h1>
        </div>
                                ...
                                @forelse  ($data as $i=>$offer)
                                    ...
                                    <tr class="{{$i%2?'bg-gray-50':'bg-white'}}">
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                            ...
                                            <x-button_icon.download wire:click="download({{$offer->id}})" />
                                            <x-button_icon.edit wire:click="edit({{$offer->id}})" />
                                            <x-button_icon.delete :object="$offer" />
                                            </div>
                                        </td>
                                    </tr>
                                @empty
