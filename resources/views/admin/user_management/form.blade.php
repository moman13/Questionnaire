                        ...
                        <div class="shadow overflow-hidden sm:rounded-md">
                            <div class="px-4 py-5 bg-white sm:p-6">
                                <div class="grid grid-cols-6 gap-6">
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="country" class="block text-sm font-medium leading-5 text-gray-700">Roles</label>
                                        <select id="country" wire:model.debounce.500ms="form.role_id" class="@error('form.role_id') border-red-300 text-red-900 placeholder-red-300 focus:border-red-300 focus:shadow-outline-red @enderror mt-1 block form-select w-full py-2 px-3 py-0 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5">

                                           @foreach($roles as $role)
                                            <option {{$role->id ==  $form['role_id']?'selected':'' }} value="{{$role->name}}">{{$role->name}}</option>
                                           @endforeach
                                            @if($status ==2)
                                                   <option selected="selected" value="">Roles select</option>
                                            @endif
                                        </select>
                                        @error('form.role_id')<span class="mt-2 text-sm text-red-600">{{ $message }}</span>@enderror
                                    </div>
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        <x-checkout :name="'status'" :value="''" :title="'active/inactive'" :livewire="'form.status'" ></x-checkout>
                                    </div>
                                </div>
                            </div>
